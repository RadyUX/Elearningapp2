...
export const ChapterList = ({
    ...
    const onDragEnd = (result: DropResult) =>{
        ...
        const [reorederedItems] = items.splice(result.source.index, 1)
        items.splice(result.destination.index,0,reorederedItems)
                                ...
                                {(provided)=> (
                                    <div className={cn("flex items-center gap-x-2 bg-[white] text-slate-700 rounded-md mb-4 text-sm",
                                        chapter.isPublished && "bg-sky-100 border-sky-200 text-sky-700"
                                            ...
                                            <div className={cn("px-2 py-3 hover:bg-slate-300 eounded-l-md transition", chapter.isPublished && "border-r-sky-200 hover:bg-sky-200")}{...provided.dragHandleProps}>
                                                <Grip/>
                                            </div>
                                            {chapter.title}
